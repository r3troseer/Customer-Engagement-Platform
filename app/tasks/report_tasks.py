...
async def run_scheduled_reports() -> None:
    ...
    """
    from app.services import notification_service, reporting_service

    async with AsyncSessionLocal() as db:
        templates = await reporting_service.get_scheduled_templates(db)
        for template in templates:
            try:
                run = await reporting_service.execute_report_run(
                    db, template.id, template.organization_id, generated_by=None
                )
                await db.commit()
        ...
        logger.info("run_scheduled_reports: processed %d templates", len(templates))
